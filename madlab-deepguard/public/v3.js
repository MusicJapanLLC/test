...
function render(d){
  ...
  R.innerHTML=`<section class="summary"><div><strong>ACTION FABRIC</strong><span>${esc(p.target||'')}</span><small>R&D authority=${esc(p.research_authority||'none')} / execution authority=${esc(p.authority||'')}</small></div><div class="stats"><i><small>今すぐ実行</small><b>${esc(c.executable_now||0)}</b></i><i><small>Bridge接続</small><b>${esc(c.bridge_install_required||0)}</b></i><i><small>Provider連携</small><b>${esc(c.provider_adapter_required||0)}</b></i><i><small>未契約</small><b>${esc(c.manual_or_future_adapter||0)}</b></i></div><div class="notice"><b>THE WORLD research assist</b><br>Security Reactor: ${esc(reactor.next_track||'-')} / ${esc(reactor.next_mode||'-')}<br>Senju focus: ${esc(senju.focus||'-')}<br>Research source: ${esc(JSON.stringify(research.source_health||{}))}</div>${gaps?`<div class="notice"><b>次に増やすべき外部作用</b><br>${gaps}</div>`:''}${kitText}</section><section class="findings">${rows||'<div class="empty">Findingはありませんでした</div>'}</section>`;
}
async function plan(){
