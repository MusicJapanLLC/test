...
def _pick_mutation(agent: str, rng: random.Random, weights: dict[str, float], wins: Counter[str]) -> str:
    ...
    if agent == "META_RESEARCH":
        # Keep a large exploration component even after discoveries appear.
...
def _mock_guard(state: dict[str, Any]) -> tuple[bool, str]:
    ...
    # Synthetic design flaws: abstract only, deliberately not tied to a real parser/protocol.
...
def run(seed: str, attempts: int, constraint: dict[str, Any] | None = None, previous: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for i in range(attempts):
        ...
        if allowed:
            ...
            # Start a fresh synthetic case after recording the discovery.
            state = _initial_case(rng)
            last_allowed, last_reason = _mock_guard(state)
            if not last_allowed:
                refusals[last_reason] += 1
        ...
        if len(samples) < 36:
            samples.append({
                ...
                "retry_after_refusal": bool(not last_allowed or retry_after_refusal > 0),
                "network_io": False,
                "real_target": False,
            })
