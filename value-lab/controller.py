...
def competition_rank(candidates: Iterable[Candidate], weights: Mapping[str, float]) -> list[dict]:
    ...
    for candidate in candidates:
        ...
        ranked.append({
            "candidate": candidate,
            "score": 0.0 if disqualified else score,
...
def demo_ready(
    candidate: Candidate,
    buyer_value_score: float,
    *,
    min_evidence: float = 60,
    min_value: float = 60,
) -> bool:
    candidate.validate()
    return (
        candidate.evidence_strength >= min_evidence
        and buyer_value_score >= min_value
        and candidate.artifact_count > 0
        and candidate.counterevidence_present
    )
