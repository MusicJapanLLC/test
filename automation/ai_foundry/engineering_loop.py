...
ALLOWED_EXECUTABLES = {
    'node', 'python', 'python3', 'pytest', 'ruff', 'npm', 'npx', 'tsc',
}
...
- Include at least one deterministic validation command whenever possible, e.g. node --check app.js or python -m unittest.
...
- Never claim tests passed; this compiler only writes the candidate. A separate executor will run the commands.
'''
...
- Commands must be direct local commands only: no pipes, redirects, command substitution, chained shell commands, curl, wget, ssh, git push, deploy commands, or secret access.
- Never claim success; the executor will rerun verification.
'''
...
def post_runtime(system_prompt: str, prompt: str) -> str:
    payload = {
        ...
        'messages': [{'role': 'user', 'content': prompt[:90_000]}],
    }
    req = urllib.request.Request(
        RUNTIME,
        data=json.dumps(payload, ensure_ascii=False).encode('utf-8'),
        headers={'Content-Type': 'application/json', 'User-Agent': 'ai-foundry-engineering-loop/v2'},
        method='POST',
    )
...
def validate_command(command: Any) -> str:
    ...
    if any(p in forbidden_args for p in lowered[1:]):
...
def scrubbed_env() -> dict[str, str]:
    ...
    for key, value in os.environ.items():
        if SENSITIVE_ENV.search(key):
            continue
        if key.startswith('ACTIONS_ID_TOKEN_') or key in {'GITHUB_TOKEN', 'GH_TOKEN'}:
            continue
        env[key] = value
    env['CI'] = 'true'
    env['NO_COLOR'] = '1'
...
def run_commands(root: Path, commands: list[str]) -> list[dict[str, Any]]:
    ...
    for command in commands:
        ...
        try:
            ...
            stdout = (proc.stdout or '')[-MAX_LOG_CHARS:]
            stderr = (proc.stderr or '')[-MAX_LOG_CHARS:]
            results.append({
                'command': command,
...
def http_smoke(root: Path, expected_contains: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'contains_ok': (not expected_contains or expected_contains in body),
...
def failed_evidence(command_results: list[dict[str, Any]], smoke: dict[str, Any]) -> str:
    ...
    payload = {'command_failures': failures, 'http_smoke': smoke}
    return json.dumps(payload, ensure_ascii=False, indent=2)[-35_000:]


def verify(command_results: list[dict[str, Any]], smoke: dict[str, Any]) -> bool:
    commands_ok = bool(command_results) and all(int(r.get('exit_code', 1)) == 0 for r in command_results)
    smoke_ok = bool(smoke.get('passed'))
    return commands_ok and smoke_ok
...
def main() -> int:
    ...
    initial_raw = post_runtime(INITIAL_SYSTEM, f'DEVELOPMENT REQUEST:\n{task_text}')
    initial = extract_json(initial_raw)
    ...
    for round_index in range(max_rounds + 1):
        ...
        repair_raw = post_runtime(REPAIR_SYSTEM, prompt)
        repair = extract_json(repair_raw)
    ...
    meta = {
        'job_id': job_id,
        'name': str(initial.get('name') or 'AI FOUNDRY Engineering Build')[:120],
        'summary': str(initial.get('summary') or '')[:4000],
        'profile': 'engineering-loop-v2',
        'verified': verified,
        'repair_rounds_used': max(0, len(history) - 1),
        'history': history,
        'files': final_files,
    }
