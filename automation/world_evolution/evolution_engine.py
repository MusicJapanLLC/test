...
FAILURES = ("timeout", "not_found", "rate_limited", "denied", "bad_hypothesis")
...
def next_generation(agents: list[Agent], count: int, seed: int, generation: int) -> list[Agent]:
    ...
    return [
        mutate(rng.choice(elites), f"g{generation:03d}-a{i:04d}", generation, rng)
        for i in range(count)
    ]
    ...
    for generation in range(generations):
        ...
        for agent in population:
            for _ in range(experiments_per_agent):
                host = rng.choice(world)
                memory.append(run_experiment(agent, host))

        ranked = sorted(population, key=lambda a: (a.score, a.successes), reverse=True)
        successes = sum(a.successes for a in population)
        failures = sum(a.failures for a in population)
        failure_histogram: dict[str, int] = {}
        for item in memory:
            if item.generation == generation and item.failure_class:
...
def main() -> None:
    ...
    for name, value, maximum in (
        ("agents", args.agents, 500),
        ("hosts", args.hosts, 5000),
        ("generations", args.generations, 50),
        ("experiments_per_agent", args.experiments_per_agent, 100),
    ):
    ...
    report = run_loop(
        ...
        experiments_per_agent=args.experiments_per_agent,
        seed=args.seed,
    )
