...
1. A recently merged lineage whose latest production Audit failed is returned to
   Fix under the same canonical lineage_id.
2. Otherwise, confirmed META findings are selected.
3. Explicit codegen task contracts are preferred; unique ordinary-code
   Surface-Scout findings may materialize a temporary pytest-backed task.

The selector itself never writes GitHub state and never broadens authority. In
GitHub Actions it may use the existing read permissions to re-audit recent
merged lineage PRs so Audit -> Fix feedback does not require a new privileged
workflow.
"""
...
def _retry_task_from_audit(audit: Mapping[str, Any], receipt: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    failed = [actor for actor, vote in votes.items() if isinstance(vote, Mapping) and not bool(vote.get("passed"))]
    ...
    goal = (
        f"Continue the same production lineage after Audit FAIL. "
        f"The previous applied attempt failed audit actors: {', '.join(failed) or 'unknown'}. "
        f"Repair the declared output so the original test contract and post-merge integrity audit pass."
    )
    if test_output:
    ...
    return {
        ...
        "test_cmd": test_cmd,
        "constraints": "Stay inside the original output file and the existing authority boundary. Preserve unrelated behavior. This is a retry of the same canonical lineage, not a new authority grant.",
        "audit_retry": True,
    }
    ...
    *,
    ...
    for audit in failed_audits:
        ...
        rows.append({
            ...
            "source": "audit_retry",
            # Existing workflow uses lineage_id as a PR-dedupe/branch key. Include a
            # unique retry suffix there, while closed_loop_lineage recomputes and
            # embeds the unchanged canonical lineage_id from detection/task/ref.
            "lineage_id": f"{canonical}-{attempt_tag}",
            "canonical_lineage_id": canonical,
            "retry": True,
...
def live_failed_audits(*, limit: int = 20, lookback_hours: int = 24) -> tuple[dict[str, Any], ...]:
    ...
    def merged_at(row: Mapping[str, Any]) -> str:
        return str(row.get("mergedAt") or "")

    prs.sort(key=merged_at, reverse=True)
    seen: set[str] = set()
