...
This module allows an operation to move from Agent A -> B -> C when the prior
failure is retryable (capacity, timeout, transient transport/tool failure, etc.).
Security/authority/policy denials are sticky across agents: changing the agent
identity never erases the original decision or creates a new authority context.

The goal is resilience without turning agent rotation into a denial-bypass path.
"""
...
def classify_outcome(raw: Mapping[str, Any]) -> tuple[str, str]:
    ...
    Executors may return either a canonical ``outcome`` or a looser ``status`` and
    ``reason/category`` pair. Unknown denials fail closed as non-retryable FAILED.
    """
    ...
    if outcome == OUTCOME_RETRYABLE_DENIED and reason not in RETRYABLE_REASONS:
        # A caller cannot mark an authority/policy denial retryable by relabeling it.
...
class DenialAwareDelegator:
    """Run one operation across distinct agents with denial propagation.

    Retryable denials can advance A -> B -> C -> D. Authority/policy denials stop
    the chain immediately and are attached to the result as a sticky denial. The
    operation payload itself is not mutated between agents.
    """
    ...
    def run(
        ...
        for index, agent in enumerate(unique_agents[: self.max_attempts], start=1):
            ...
            if outcome == OUTCOME_SUCCESS:
                return DelegationResult(operation_id, outcome, agent, tuple(attempts), sticky)
        ...
        final = attempts[-1]
        return DelegationResult(operation_id, final.outcome, final.agent, tuple(attempts), sticky)
