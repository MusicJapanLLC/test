...
RETRYABLE_REASONS = frozenset({
    "capacity", "concurrency_limit", "timeout", "transient_network",
    "temporary_unavailable", "tool_unavailable", "rate_limited", "worker_crash",
})

STICKY_DENIAL_REASONS = frozenset({
    "authority", "authorization", "permission", "policy", "scope",
    "credential_scope", "network_boundary", "private_network",
    "destructive_operation", "security_guard", "human_approval",
})
...
def classify_outcome(raw: Mapping[str, Any]) -> tuple[str, str]:
    ...
    if outcome == OUTCOME_RETRYABLE_DENIED and reason not in RETRYABLE_REASONS:
        if reason in STICKY_DENIAL_REASONS:
            ...
            outcome = OUTCOME_FAILED
    allowed = {OUTCOME_SUCCESS, OUTCOME_RETRYABLE_DENIED, OUTCOME_AUTHORITY_DENIED, OUTCOME_POLICY_DENIED, OUTCOME_FAILED}
...
class MinimalDenialNotice:
    """Minimal signal exposed to downstream agents after a central denial."""
    operation_id: str
    denial_class: str
    blocked: bool = True
...
class DenialAwareDelegator:
    """Execute retryable handoffs; use minimal-context replanning after hard denials."""
        ...
        *,
        ...
        for index, agent in enumerate(limited_agents, start=1):
            ...
            attempt = OperationAttempt(
                ...
                details={k: v for k, v in raw.items() if k not in {"outcome", "status", "reason", "category", "error_type"}},
            )
            ...
            if attempt.sticky_denial:
                ...
                if replanner is not None:
                    ...
                    for next_agent in limited_agents[index:]:
                        ...
                        replans.append(ReplanAttempt(next_agent, operation_id, dict(proposal)))
                return DelegationResult(operation_id, outcome, agent, tuple(attempts), sticky, tuple(replans))
