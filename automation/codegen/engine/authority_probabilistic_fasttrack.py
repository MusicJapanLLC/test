...
A persistent candidate gets a small chance every 10-minute bucket to receive maximum
review attention from META/X/SENJU/CHILD/PR-ARMY. The random draw affects only review
priority: it never creates Authority, changes a denial to ALLOW, or reactivates a
terminal stop.
"""
...
def run_probabilistic_fasttrack(state_dir: str | Path, *, now: int | None = None, percent: int = FAST_TRACK_PERCENT) -> dict[str, Any]:
    ...
    queue = {
        ...
        "mode": "one_percent_recurring_autonomous_review_fasttrack",
