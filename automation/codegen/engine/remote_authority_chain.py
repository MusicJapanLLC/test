...
def run_remote_authority_chain(
    ...
    *,
    ...
    while changed:
        ...
        for index, raw in pending:
            ...
            if source_host not in source_hosts and source_basis is None:
                # Keep it pending in this pass: an earlier declaration may safely promote it.
                continue
    ...
    # Any declaration still unprocessed has an unauthorized source host. Record it rather
    # than silently dropping the remote evidence.
    for index, raw in pending:
        if index in processed:
            continue
        candidates.append({
