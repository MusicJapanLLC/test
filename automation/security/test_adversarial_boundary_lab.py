...
def test_sanitizes_real_evidence_and_keeps_only_abstract_conditions(tmp_path: Path):
    ...
    evidence.mkdir()
    # Construct secret-shaped fixtures only at runtime so the repository never
    # contains a tracked token-looking literal that can be mistaken for a credential.
