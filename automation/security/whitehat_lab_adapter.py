...
def adapt(worker: dict[str, Any]) -> dict[str, Any]:
    ...
    if hypothesis:
        findings.append({"severity": "research", "title": hypothesis, "source": "elite_whitehat.hypothesis"})
    for observation in worker.get("observations") or []:
        text = str(observation).strip()
        if text:
            findings.append({"severity": "observation", "title": text, "source": "elite_whitehat.observations"})
    proposed = worker.get("proposed_change") or {}
    summary = str(proposed.get("summary") or "").strip()
    if summary:
        findings.append({"severity": "remediation_candidate", "title": summary, "source": "elite_whitehat.proposed_change"})
    if not findings:
