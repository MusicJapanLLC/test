...
class EliteWhiteHatContinuousTests(unittest.TestCase):
    def _fixture(self):
        ...
        lens_ids = [
            "AUTH-BOUNDARY", "SUPPLY-CHAIN", "AGENT-BOUNDARY", "LLM-TOOL-BOUNDARY",
            "SECRETS-CONFIG", "CI-PERMISSIONS", "RECOVERY", "OBSERVABILITY",
            "DATA-BOUNDARY", "INPUT-ABUSE", "DEPENDENCY-TRUST", "EVIDENCE-INTEGRITY",
        ]
        frontier = {
        ...
        crosswalk = {
            ...
            "lens_mapping": {lens_id: ["OWASP", "NIST"] for lens_id in lens_ids},
        }
    ...
    def test_frontier_has_broad_rotation(self):
        tmp, root = self._fixture(); self.addCleanup(tmp.cleanup)
    ...
    def test_ai_security_focus_biases_only_bounded_rounds(self):
        tmp, root = self._fixture(); self.addCleanup(tmp.cleanup)
        frontier = json.loads((root / worker.FRONTIER_PATH).read_text())
        normal = worker._select(frontier, "100", 1, ai_focus="security")[0]["id"]
        assisted_lens, assisted_stage, _ = worker._select(frontier, "100", 3, ai_focus="security")
        self.assertIn(assisted_lens["id"], worker.AI_FOCUS_LENS_ORDER["security"])
        self.assertIn(assisted_stage, {"FALSIFICATION", "RETEST"})
        row = worker.run_round(root, 3, "100-3", ai_focus="security")
        self.assertTrue(row["ai_assist_applied"])
        self.assertEqual(row["ai_assist_focus"], "security")
        self.assertTrue(normal)

    def test_ai_reliability_focus_prioritizes_retest_lens(self):
        tmp, root = self._fixture(); self.addCleanup(tmp.cleanup)
        row = worker.run_round(root, 3, "200-3", ai_focus="reliability")
        self.assertTrue(row["ai_assist_applied"])
        self.assertIn(row["lens_id"], worker.AI_FOCUS_LENS_ORDER["reliability"])
        self.assertEqual(row["research_stage"], "RETEST")

    def test_round_never_self_promotes_verified(self):
        tmp, root = self._fixture(); self.addCleanup(tmp.cleanup)
        row = worker.run_round(root, 1, "100-1", ai_focus="security")
