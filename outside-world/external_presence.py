...
def evaluate_intents(intents: list[dict[str, Any]], policy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, intent in enumerate(intents):
        ...
        results.append({"intent_id": intent.get("intent_id") or f"intent-{index+1}", "citizen_id": intent.get("citizen_id"), "action": intent.get("action"), "platform": intent.get("platform"), **result})
    return results
...
def render_report(plan: dict[str, Any], decisions: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"**Principle:** `{plan.get('principle')}`", "", "## Mission lanes",
    ]
