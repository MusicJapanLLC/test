...
def _mission_gate(lane: dict[str, Any]) -> dict[str, Any]:
    if not bool(lane.get("write_intent")):
        return {"required": False, "execution_default": "AUTO_PUBLIC_OR_AUTHORIZED_READ"}
    return {
        "required": True,
...
def build_plan(snapshot: dict[str, Any], policy: dict[str, Any], seed: str) -> dict[str, Any]:
    ...
    return {
        ...
        "invariants": {
            ...
            "write_missions_require_adapter_confirmation": all(m["required_gate"].get("execution_default") == "DRAFT_ONLY_UNTIL_ADAPTER_CONFIRMS" for m in write_missions),
...
def evaluate_intents(intents: list[dict[str, Any]], policy: dict[str, Any]) -> list[dict[str, Any]]:
    return [{"intent_id": intent.get("intent_id") or f"intent-{i+1}", "citizen_id": intent.get("citizen_id"), "action": intent.get("action"), "platform": intent.get("platform"), **decide_intent(intent, policy)} for i, intent in enumerate(intents)]
...
def render_report(plan: dict[str, Any], decisions: list[dict[str, Any]]) -> str:
    ...
    lines = ["# THE WORLD — External Presence", "", f"**Citizens projected:** {plan['profiles_count']}", f"**Missions issued:** {plan['missions_count']}", f"**Read missions:** {plan.get('read_missions_count', 0)}", f"**Write intents:** {plan.get('write_missions_count', 0)}", f"**Principle:** `{plan.get('principle')}`", "", "## Mission lanes"]
    lines += [f"- {lane}: {count}" for lane, count in sorted(lane_counts.items())]
    lines += ["", "## Identity / auth", "- Every citizen has a stable presence id.", "- Credentials are secret-manager reference slots only; passwords/tokens are not written into GitHub or mission payloads.", "- Public/authorized reads are proactive.", "- Authorized write missions are generated proactively, but remain DRAFT_ONLY until the adapter confirms terms, connector authority, target scope, identity, availability and reversibility.", "", "## LIMITLESS / Reality", "- Each mission inherits LIMITLESS and the ACT -> VERIFY -> LOG -> LEARN -> IMPROVE cycle.", "- Observation should be converted into an artifact, experiment, customer value, operational proof, or revenue-distance reduction when possible."]
    if decisions:
        counts: dict[str, int] = {}
        for d in decisions:
            counts[d["decision"]] = counts.get(d["decision"], 0) + 1
        lines += ["", "## Intent decisions"] + [f"- {name}: {count}" for name, count in sorted(counts.items())]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    if args.intents and Path(args.intents).exists():
        intents = list(load_json(args.intents).get("intents") or [])
    decisions = evaluate_intents(intents, policy)
    Path(args.json).write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    Path(args.decisions).write_text(json.dumps({"schema": "the-world-external-action-decisions/v1", "generated_at": datetime.now(timezone.utc).isoformat(), "decisions": decisions}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    Path(args.report).write_text(render_report(plan, decisions), encoding="utf-8")
    print(json.dumps({"population": plan["population_discovered"], "profiles": plan["profiles_count"], "missions": plan["missions_count"], "read_missions": plan.get("read_missions_count", 0), "write_missions": plan.get("write_missions_count", 0), "decisions": len(decisions)}, ensure_ascii=False))
    return 0
