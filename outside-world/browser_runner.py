...
async def observe(page: Any, task: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        response = await page.goto(task["url"], wait_until="domcontentloaded", timeout=30_000)
...
async def run(tasks: list[dict[str, Any]], policy: dict[str, Any], limit: int) -> list[dict[str, Any]]:
    ...
    async with async_playwright() as p:
        ...
        context = await browser.new_context(
            user_agent="TheWorld-ResearchBrowser/1.0",
        ...
        for task in selected:
            ...
            results.append(await observe(page, task))
            await page.close()
        await context.close()
        await browser.close()
    return results
...
def main() -> int:
    ...
    results = asyncio.run(run(task_doc.get("tasks", []), policy, max(1, args.limit)))
    out = {
    ...
    print(json.dumps({"observed": len(results), "ok": sum(1 for r in results if r.get('status') == 'OK')}, ensure_ascii=False))
    return 0
