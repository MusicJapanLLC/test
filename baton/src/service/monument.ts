...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    // 一つひとつがわずかに息をする
    float breath = sin(uTime * 0.6 + aSeed * 6.28318) * 0.04;
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    vec3 base = mix(uPrimary, uAccent, clamp(vSeed * 0.75 + shift * 0.25, 0.0, 1.0));

    vec3 color = mix(base * 0.72, base, lambert);
    color = mix(color, uBg, 0.14);
    color = mix(color, uAccent, fresnel * 0.7);

    float alpha = 0.30 + lambert * 0.30 + fresnel * 0.34;
...
export function createMonument(kind: MonumentKind, theme: Theme, low: boolean): Monument {
  ...
  return {
    ...
    update(elapsed, scroll, mouse) {
      ...
      mesh.rotation.y = elapsed * 0.09 + mouse.x * 0.5 + scroll * 0.8;
      mesh.rotation.x = mouse.y * -0.3 + Math.sin(elapsed * 0.18) * 0.06;
    },
    dispose() {
      geometry.dispose();
      material.dispose();
