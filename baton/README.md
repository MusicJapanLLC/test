...
合同会社Music Japanが提携する7つのBtoBサービスを紹介するハブ型サイト。

- ハブページ1枚（Baton）＝ 7サービスへの目次
- サービス紹介ページ7枚 ＝ それぞれがミニLPとして完結
- 各ページ末尾に、カード送り式のアンケート（4問＋属性）
- 回答は Google Apps Script 経由でスプレッドシートへ

## 構造の考え方

**7ページを7回作っていない。** 1つのテンプレート × 7つの設定で動く。

色・ロゴ・コピー・実績数値・設問・送信先は、すべて `src/data/services.ts` にある。
8個目（LegalOn など）を足すときの作業は3つだけ:
