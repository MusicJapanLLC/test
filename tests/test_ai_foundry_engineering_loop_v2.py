...
class EngineeringLoopV2Tests(unittest.TestCase):
    ...
    def test_safe_relpath_rejects_escape(self):
        with self.assertRaises(ValueError):
            mod.safe_relpath('../outside.txt')
        with self.assertRaises(ValueError):
            mod.safe_relpath('/absolute.txt')
        with self.assertRaises(ValueError):
            mod.safe_relpath('.git/config')
    ...
    def test_command_policy_blocks_shell_and_publish(self):
        for command in (
            ...
            with self.subTest(command=command):
                with self.assertRaises(ValueError):
                    mod.validate_command(command)

    def test_normalize_files_blocks_duplicate_and_size_escape(self):
        with self.assertRaises(ValueError):
            mod.normalize_files([
    ...
    def test_scrubbed_env_removes_common_secret_names(self):
        ...
        try:
            ...
            os.environ['NORMAL_VALUE'] = 'ok'
            env = mod.scrubbed_env()
            self.assertNotIn('GITHUB_TOKEN', env)
            self.assertNotIn('OPENAI_API_KEY', env)
            self.assertEqual(env.get('NORMAL_VALUE'), 'ok')
        finally:
            os.environ.clear()
