...
def main() -> int:
    ...
    if sample:
        ...
        for target in targets:
            ...
            method = "HEAD" if "HEAD" in methods else "GET" if "GET" in methods else "OPTIONS"
            try:
                outcome = transport.execute(url, method=method)
                receipts.append(
                    {
                        "host": host,
                        "url": url,
    ...
    live_doc = {
        ...
        "receipts": receipts,
        "execution_boundary": "existing exact-host Authority; GET/HEAD/OPTIONS only",
    }
