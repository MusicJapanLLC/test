...
This production helper sits after Owner-scope negotiation and also acts as a
bidirectional coordination layer. It consumes structured negotiation intelligence from
the local Owner-scope lane and the shared Root Authority negotiation memory, scores what
should move next, and publishes precise feedback/handoff records back to that fabric.
...
from senju.meta.standing_authorization import StandingAuthorization, load_registry, renew_operational_lease
from senju.owner_scope_negotiation import DECISION_MEMBERS, OwnerExpansionEnvelope, _ballots_for, build_scope_proposals
...
def _standing_for(
    ...
    if not rows:
        return None
    # Prefer the valid standing record that covers the largest part of the requested
    # method set. The resulting operational lease is still same-or-narrower.
    return sorted(
        rows,
        key=lambda row: (
    ...
    *,
    ...
    for proposal in proposals:
        ...
        packet = {
            ...
            "standing_authorization_reference": standing_record.authorization_reference if standing_record else None,
    ...
    _write(out_dir / "promotion_packets.json", {"schema": PACKET_SCHEMA, "packets": ordered_packets})
