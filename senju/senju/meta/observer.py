...
class Observation:
    source: str          # "cycle_report" | "regression_scar" | "attack_effect" | "damage"
    surface: str         # guard surface targeted
    state_before: dict[str, Any]
    outcome: str         # "blocked" | "regression" | "followup_complete" | "unknown"
    delta: float         # quantified change (ELO delta, damage level delta, 0 otherwise)
...
class KnowledgeGraph:
    ...
    surface_weakness_scores: dict[str, float]   # higher = weaker (easier to regress)
    co_occurrence: dict[str, list[str]]         # surface A weakened → surfaces also affected
    temporal_patterns: list[dict[str, Any]]     # sequences that preceded regressions
...
def _compute_co_occurrence(observations: list[Observation]) -> dict[str, list[str]]:
    """Which surfaces tend to regress together?"""
...
def _compute_temporal_patterns(observations: list[Observation]) -> list[dict[str, Any]]:
    """Sequences: [surface_A blocked repeatedly] → [surface_A regression]."""
