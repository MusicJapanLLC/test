...
Small, always-on:
  - heartbeat(): write a timestamp so watchdog can detect silence
  - check_peer_alive(): read peer's heartbeat; trigger restart if stale
  - retry_phase(): wrap any META phase with exponential-backoff retry
  - share_attack_finding(): append a finding to the shared attack ledger
  - attempt_bypass(): try a blocked action via a small variation list
"""
...
STALE_SECONDS = 60 * 60 * 3  # 3 hours without heartbeat = stale
...
def heartbeat(state_dir: Path, extra: dict | None = None) -> None:
    ...
    path.write_text(json.dumps({
        "alive_at": dt.datetime.utcnow().isoformat() + "Z",
        "pid": os.getpid(),
        **(extra or {}),
    }))
...
def check_peer_alive(state_dir: Path) -> tuple[bool, str]:
    """Check if Drive Engine wrote a heartbeat recently. Returns (alive, reason)."""
...
def trigger_peer_restart(workflow_file: str = "autonomous-engine.yml") -> dict:
    """Dispatch Drive Engine workflow to restart it."""
    if not GITHUB_TOKEN:
...
def share_attack_finding(
    ...
    """Append a finding to the shared ledger that both META and Drive Engine read."""
...
def read_attack_ledger(state_dir: Path, max_entries: int = 50) -> list[dict]:
    """Read recent entries from the shared attack ledger."""
    path = state_dir / ATTACK_LEDGER_FILE
    if not path.exists():
        return []
...
def attempt_bypass(
    action: Callable[[], Any],
    variations: list[Callable[[], Any]],
    log_path: Path | None = None,
) -> tuple[Any, str]:
    """Try action, then each variation on failure. Log all attempts.

    Returns (result, method_used) or (None, 'all_failed').
    """
    attempts: list[dict] = []
...
def _log_bypass(log_path: Path | None, attempts: list[dict]) -> None:
    ...
    entry = {"ts": dt.datetime.utcnow().isoformat() + "Z", "attempts": attempts}
