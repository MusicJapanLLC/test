"""Hypothesis Validator — tracks prediction → experiment → outcome → confidence update.

Every hypothesis META generates gets tracked across cycles:
  PENDING → TESTED → CONFIRMED | REFUTED | PARTIAL

Confirmed hypotheses become permanent attack DNA.
Refuted hypotheses inform META's own self-rewrite proposals.
"""
...
class TrackedHypothesis:
    ...
    confidence: float
    status: str = "pending"           # pending | tested | confirmed | refuted | partial
...
def save_tracker(tracker: dict[str, TrackedHypothesis], state_dir: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {k: dataclasses.asdict(v) for k, v in tracker.items()},
            ensure_ascii=False,
            indent=2,
        )
    )
...
def register(hypotheses: list[Any], tracker: dict[str, TrackedHypothesis]) -> int:
    """Add new hypotheses to the tracker (skip already-tracked ones)."""
    added = 0
    for h in hypotheses:
...
def update_from_cycle(
    tracker: dict[str, TrackedHypothesis],
    cycle_report: dict[str, Any],
) -> list[str]:
    """Match cycle results against pending hypotheses and update status."""
    resolved: list[str] = []
    ...
    for hid, hyp in tracker.items():
        ...
        relevant_regressions = [s for s in hyp.surfaces if s in regressions]

        if hyp.predicted_outcome in ("regression", "controlled_regression"):
            if relevant_regressions:
                ...
                hyp.resolved_at = dt.datetime.utcnow().isoformat() + "Z"
                resolved.append(hid)
        elif hyp.predicted_outcome == "co_regression":
            if len(relevant_regressions) >= 2:
                hyp.status = "confirmed"
                hyp.confidence = min(0.99, hyp.confidence + 0.15)
                hyp.resolved_at = dt.datetime.utcnow().isoformat() + "Z"
                resolved.append(hid)

        hyp.test_results.append({
            "cycle": hyp.cycles_elapsed,
            "regressions_seen": relevant_regressions,
            "status_after": hyp.status,
        })
...
def summarize(tracker: dict[str, TrackedHypothesis]) -> dict[str, Any]:
    ...
    return {
        ...
        "pending": len(pending),
        "top_confirmed": [
            {"id": h.hypothesis_id, "confidence": h.confidence, "surfaces": h.surfaces}
