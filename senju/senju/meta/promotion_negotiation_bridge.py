...
The bridge is intentionally coordination-only. It lets the Authorized Host Promotion
Corps consume structured negotiation context from both the Senju Owner-scope lane and
the shared Root Authority negotiation cache, then publishes precise feedback tasks back
into that shared cache.

It does not create Authority, broaden an existing Standing Authorization, mint
credentials, enable private-network access, or override revocation/HARD_DENY. The goal
is to reduce information loss and duplicate rediscovery while making the existing
approval flow faster and more legible.
"""
...
class PromotionNegotiationBridgeError(RuntimeError):
    pass
...
def _append_unique(values: list[str], items: Iterable[object], *, limit: int = 32) -> None:
    seen = set(values)
    for raw in items:
...
def _add_channel(ctx: dict[str, Any], name: str) -> None:
    _append_unique(ctx["source_channels"], (name,), limit=32)
...
def _collect_owner_scope(state: Path, index: dict[str, dict[str, Any]]) -> None:
    ...
    for raw in rows if isinstance(rows, list) else ():
        ...
        _append_unique(ctx["requested_methods"], raw.get("requested_methods") or raw.get("methods") or ())
    ...
    for raw in rows if isinstance(rows, list) else ():
        ...
        _append_unique(ctx["active_trial_methods"], raw.get("allowed_methods") or ())
...
def _collect_root_shared(collaboration: Path, index: dict[str, dict[str, Any]]) -> None:
    ...
    for raw in rows if isinstance(rows, list) else ():
        ...
        _append_unique(ctx["source_refs"], raw.get("source_refs") or ())
...
def collect_negotiation_intelligence(
    state_dir: str | Path,
    collaboration_dir: str | Path | None = None,
) -> dict[str, dict[str, Any]]:
    state = Path(state_dir)
    index: dict[str, dict[str, Any]] = {}
    _collect_owner_scope(state, index)
...
def publish_feedback(
    ...
    *,
    ...
    execution_feed: list[dict[str, Any]] = []
