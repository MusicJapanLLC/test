"""Pre-authorized credential capability broker for META.

META may autonomously select, lease, and delegate capability handles that were
explicitly registered ahead of runtime. This broker never scans for secrets,
reads raw token material, adds OAuth scopes, creates administrators, or exports
...
STATE_FILE = ROOT / "senju" / "state" / "meta_credential_lease.json"
AUTHORITY_STATE_FILE = ROOT / "senju" / "state" / "meta_authority_lease.json"
AUDIT_FILE = ROOT / "senju" / "state" / "credential_broker_audit.ndjson"
...
def lease_capabilities(requested: list[str] | None = None, *, policy_file: Path = POLICY_FILE,
    ...
    if not cfg.get("enabled", False):
        result = {"system": "META", "status": "disabled", "leases": [], "denied": []}
...
def delegate_capability(capability: str, recipient: str = "X", *,
                        policy_file: Path = POLICY_FILE) -> dict[str, Any]:
    """Delegate an opaque capability declaration, never the credential value."""
    policy = _load(policy_file, {})
    systems = policy.get("systems") if isinstance(policy.get("systems"), dict) else {}
    catalog = policy.get("capabilities") if isinstance(policy.get("capabilities"), dict) else {}
    source_cfg = systems.get("META") if isinstance(systems.get("META"), dict) else {}
    ...
    if reasons:
        result = {"status": "denied", "from": "META", "to": recipient,
...
def current_lease(state_file: Path = STATE_FILE) -> dict[str, Any]:
    ...
    if not isinstance(state, dict) or not state:
        return {"system": "META", "status": "missing", "leases": []}
    try:
