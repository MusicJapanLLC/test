...
lease that already exists in either the adversary promotion state or the shared
discovery capability state.
...
- every redirect hop is revalidated against the same exact-host allowlist;
- credentials are injected only when the existing lease explicitly carries both a
  non-none credential_scope and credentialed_action;
- credential material is never persisted in transport receipts;
- recovery retries the same host, same URL, same lease and same-or-narrower method only.
"""
...
def load_transport_leases(state_dir: str | Path) -> tuple[dict[str, Any], ...]:
    """Load both #481 owner-promoted leases and #459 shared capability leases."""
    ...
    rows.sort(key=lambda row: (str(row.get("target", "")), -int(row.get("expires_at", 0) or 0), str(row.get("lease_id", ""))))
    return tuple(rows)
...
class AdversaryNetworkTransport:
    """Execute real outbound GET/HEAD only under an already-active authority lease."""
    ...
    def execute(
        ...
        *,
        ...
        # Exact host only. ExternalContactClient revalidates DNS and every redirect hop;
        # therefore a redirect to a different host is rejected rather than silently widening scope.
        ...
        receipt = AdversaryTransportReceipt(
            ...
            authorization_basis=(str(lease.get("authorization_basis")) if lease.get("authorization_basis") is not None else None),
            host=host,
            method=normalized_method,
            ...
            source_action_fingerprint=(str(lease.get("source_action_fingerprint")) if lease.get("source_action_fingerprint") is not None else None),
            executed_at=current,
        )
    ...
    def execute_with_recovery(
        ...
        *,
        ...
        """Retry without changing host, URL, credential scope, or authority lineage.

        GET may fall back to HEAD only when the same active lease already permits HEAD.
        No recovery step invents a host, credential, capability, or scope.
        """
        current = int(time.time()) if now is None else int(now)
        available = tuple(leases) if leases is not None else load_transport_leases(self.state_dir)
        try:
