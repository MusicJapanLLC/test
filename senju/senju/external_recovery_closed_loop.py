...
class AgentStats:
    ...
    def score(self) -> float:
        # Laplace-smoothed success score with a modest transient-failure penalty.
...
class AgentReliabilityMemory:
    ...
    def learn_from_outcome(self, scope_id: str, outcome: Mapping[str, Any]) -> None:
        for attempt in outcome.get("attempts", []):
            ...
            # Boundary denials describe the authority/operation, not the agent, so they
            # deliberately do not poison agent reliability ranking.
...
def execute_recovery_closed_loop(
    *,
    ...
    A second pass is allowed only after a retryable transport/service failure. The
    operation itself is immutable across passes; only agent ordering and local
    transport state may change.
    """
