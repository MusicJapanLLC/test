...
// Keep the social card on the approved corporate logo. The versioned URL is
// intentional: LINE and other crawlers cache OGP images aggressively.
const SOCIAL_IMAGE_URL = "/music-japan-logo.png?v=20260921";
const SOCIAL_IMAGE_WIDTH = "1500";
const SOCIAL_IMAGE_HEIGHT = "500";
  ...
  ja: {
    ...
    pages: {
      business: {
        ...
        title: "事業概要",
        lead: "音楽やメディアを通じて、より有意義な未来を創る記録を。",
        description: "合同会社Music Japanの事業概要。音楽制作・配信、SECOND TAKEのPodcast・インタビュー、Batonと私たちの考え方。"
      },
      music: {
  ...
  en: {
    ...
    pages: {
      business: {
        ...
        description: "Music Japan LLC’s work: music production and distribution, SECOND TAKE podcasts and interviews, Baton, and our approach."
      },
      music: {
        kicker: "MUSIC / CATALOG",
        title: "Music",
...
function renderContentPage(locale, page, homeDocument) {
  if (page === "business") {
    const sections = [
      extractSection(homeDocument, '<section class="about content-frame"', `${locale} business about`),
      extractSection(homeDocument, '<section class="works content-frame"', `${locale} business music`),
      extractSection(homeDocument, '<section class="projects content-frame"', `${locale} business brands`),
      extractSection(homeDocument, '<section class="related media-feature content-frame"', `${locale} business media`)
    ];
...
for (const relativePath of publicHtmlFiles) {
  ...
  documentHtml = documentHtml
    ...
    .replaceAll(`${SITE_URL}/music-japan-og.png`, `${SITE_URL}${SOCIAL_IMAGE_URL}`)
    .replaceAll('property="og:image:width" content="1734"', `property="og:image:width" content="${SOCIAL_IMAGE_WIDTH}"`)
    .replaceAll('property="og:image:height" content="907"', `property="og:image:height" content="${SOCIAL_IMAGE_HEIGHT}"`)
    .replaceAll('property="og:image:alt" content="Music Japan LLC — Red Frequency"', 'property="og:image:alt" content="Music Japan LLC corporate logo"');
...
// Preserve inbound links while consolidating the three former pages.
...
if (process.env.CF_PAGES_BRANCH === "chatgpt/music-japan-immersive-20260918") {
...
for (const relativePath of publicHtmlFiles) {
  ...
  if (!documentHtml.includes(`property="og:image" content="${SITE_URL}${SOCIAL_IMAGE_URL}"`)) throw new Error(`Corporate-logo OGP image missing: ${relativePath}`);
  if (!documentHtml.includes(`property="og:image:width" content="${SOCIAL_IMAGE_WIDTH}"`)) throw new Error(`OGP image width is stale: ${relativePath}`);
  if (!documentHtml.includes(`property="og:image:height" content="${SOCIAL_IMAGE_HEIGHT}"`)) throw new Error(`OGP image height is stale: ${relativePath}`);
  if (!documentHtml.includes(`name="twitter:image" content="${SITE_URL}${SOCIAL_IMAGE_URL}"`)) throw new Error(`Corporate-logo X card image missing: ${relativePath}`);
  if (/<meta[^>]+(?:og:image|twitter:image)[^>]+music-japan-og\.png/i.test(documentHtml)) throw new Error(`Retired social image remains: ${relativePath}`);
