...
function renderThreads(){
  ...
  threads.forEach(t=>{
    ...
    row.addEventListener('click',e=>{if(e.target.classList.contains('thread-delete'))return;activeId=t.id;renderAll(true)});
...
function renderMessages(follow=false){
  ...
  if(!t||!t.messages.length){box.innerHTML='<div class="welcome"><h3>AI FOUNDRY CORE</h3><p>AI開発専用の3ペインIDE。中央で設計・実装・評価・デバッグを会話し、右の <code>RUN BUILD PIPELINE</code> で AI仕様生成 → Smoke Test → 実行URL発行まで進めます。</p></div>';if(follow)scrollMessagesToBottom();return}
...
function renderAll(follow=false){renderThreads();renderMessages(follow)}
...
async function send(){
  ...
  try{const r=await call('chat',{messages:t.messages});t.messages.push({role:'assistant',content:r.text});t.updatedAt=Date.now();saveThreads();renderAll(true);log(`chat complete · ${r.route||r.model||'runtime'}`)}
...
if(!activeId)newThread();else renderAll(true);
